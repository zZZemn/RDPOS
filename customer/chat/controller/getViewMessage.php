...
$query = "SELECT m.*, a.emp_image, a.acc_fname, a.acc_lname,a.acc_type
    ...
    WHERE (m.mess_reciever_id = $session_acc_id) OR (m.mess_sender_id = $session_acc_id)
    ";
    // WHERE (m.mess_sender_id = $session_acc_id AND m.mess_reciever_id = $account_id) OR (m.mess_sender_id = $account_id AND m.mess_reciever_id = $session_acc_id)
...
if ($result) {
    while ($row = mysqli_fetch_assoc($result)) {
        $get_sender_information = mysqli_query($connections, "SELECT * FROM account WHERE acc_id = '$session_acc_id'");
...
//$sql = "UPDATE messages SET mess_seen = 2 WHERE mess_sender_id = '$session_acc_id'";
