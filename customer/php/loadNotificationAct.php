...
while ($row = mysqli_fetch_assoc($view_query)) {
    ...
    $get_senderDetails = mysqli_query ($connections,"SELECT * FROM account where acc_id='$mess_sender_id' ");
    ...
    } else if ($acc_type == "admin" || $mess_user_type == "delivery") {
        echo "<div class='media-body flex-grow-1' onclick=\"window.location.href='chat.php?account_id=$mess_sender_id';\" style='cursor: pointer;'>";
    } else {
 ...
 $mess_seen;
    ...
    if($mess_seen == 2){
        $messages_log="<span class='noti-details'><br>$act_activity</span>";//ordinary
    }else{
        $messages_log="<span class='noti-title' style='font-weight: bold;'><br>$act_activity</span>";//highlights
    }
