...
if (isset($_SESSION["acc_id"])) {
    ...
    $stmt = $connections->prepare("SELECT id, user_complete_address,user_address_code ,user_add_Default_status FROM account 
        LEFT JOIN user_address ON account.acc_code = user_address.user_acc_code 
        LEFT JOIN tbl_address ON tbl_address.address_code = user_address.user_address_code 
        WHERE account.acc_id = ?
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $user_address_code=$row["user_address_code"];
        $user_add_Default_status = $row["user_add_Default_status"];
        

        $selected = ($user_complete_address == $address) ? 'selected' : '';

        $html .= '<option ' . $selected . ' value="' . $id . '" data-barangaycode="' . $user_address_code . '">' . ($user_add_Default_status == 1 ? ' (Default) ' : '') . $address . '</option>';
