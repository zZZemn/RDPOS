...
function validateForm() {
    ...
    if ($criticalLevel.val() === "") {
     ...
     if (descriptionValue === "") {
         ...
         allFieldsValid = false;
         $errorDescription.text("Description must not exceed 300 characters.");
         $errorDescription.show();
     }
 ...
 if (allFieldsValid) {
    ...
    $productName.add($unit).add($category).add($criticalLevel).add($voucherType).add($currentPrice).add($productImage).on('input', validateForm);
