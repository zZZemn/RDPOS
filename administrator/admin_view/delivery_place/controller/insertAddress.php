...
if ($count > 0) {
    ...
    $deliveryAllowed = $deliveryAllowed == "true" ? 1 : 0;
    $paymentfirstAllowed = $paymentfirstAllowed == "true" ? 1 : 0;

    $insertQuery = "INSERT INTO tbl_address (address_code, address_complete_name, address_rate, address_status, address_cod, address_paynow, address_date_added, address_display_status) 
    VALUES (?, ?, ?, ?, ?, ?, NOW(), 1)";
    ...
    mysqli_stmt_bind_param($stmt, "ssdiii", $brgy_code, $completeAddress, $shipping, $activeStatus, $deliveryAllowed, $paymentfirstAllowed);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
