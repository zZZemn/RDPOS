...
$("#stockTableBody").on("change", "input:checkbox", function () {
    ...
    var allChecked = $("#stockTableBody").find("input:checkbox:not(#AllCheckbox)").length ===
                     $("#stockTableBody").find("input:checkbox:checked:not(#AllCheckbox)").length;
...
var updateInterval = setInterval(updateStockTable, 2000);
    ...
    $(document).on('click', '.editTogler', function() {
        ...
        var db_prod_expirationStatus =$(this).attr("data-db_prod_expirationStatus")
        ...
        if(db_prod_expirationStatus=="N/A"){
            ...
            $("#updateexpiDate").val(db_s_expiration)
                ...
                $('#updateproductname').html(
                    ...
                    ('Current price:' + (db_prod_currprice ? '₱' + Number(db_prod_currprice).toFixed(2) + '<br>' : ''))
                );
