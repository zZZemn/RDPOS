...
function validateForm() {
    ...
    if ($criticalLevel.val() === "") {
        ...
        allFieldsValid = false;
        $errorCriticalLevel.text("Critical Level must be a positive integer with no special characters or decimals.");
        $errorCriticalLevel.show();
    } else {
        ...
        if (criticalLevelValue < 1 || criticalLevelValue > 10000) {
            allFieldsValid = false;
            $errorCriticalLevel.text("Critical Level must be between 1 and 10,000.");
            $errorCriticalLevel.show();
        }
    }
        ...
        if (descriptionValue === "") {
            ...
            allFieldsValid = false;
            $errorDescription.text("Description must not exceed 255 characters.");
            $errorDescription.show();
        }
    ...
    } else if (isNaN($currentPrice.val()) || parseFloat($currentPrice.val()) <= 0) {
        allFieldsValid = false;
        $errorCurrentPrice.text("Current price must be a valid number greater than 0.");
        $errorCurrentPrice.show();
    } else if (!/^\d{1,6}(\.\d{1,2})?$/.test($currentPrice.val())) {
        allFieldsValid = false;
        $errorCurrentPrice.text("Current price must have up to 6 digits before the decimal point and up to 2 digits after the decimal point.");
        $errorCurrentPrice.show();
 ...
 if (imageFileName === "") {
     ...
     if (validExtensions.indexOf(fileExtension) === -1) {
         allFieldsValid = false;
         $errorProductImage.text("Invalid file format. Only JPG, JPEG, PNG, or GIF files are allowed.");
         $errorProductImage.show();
         alertify.error("Invalid file format. Only JPG, JPEG, PNG, or GIF files are allowed");
     } else {
         // Display the uploaded image
         var productImagePreview = document.getElementById('productImagePreview');
         productImagePreview.style.display = 'block';
         productImagePreview.src = URL.createObjectURL($productImage[0].files[0]);
     }
 }
 ...
 if (allFieldsValid) {
    ...
    $productName.add($category).add($criticalLevel).add($voucherType).add($currentPrice).add($productImage).on('input', validateForm);
