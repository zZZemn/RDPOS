...
$pname = $unit = $pcat = $pcritical = $pDescript = $pVouch  = $pCprice = $pImg = "";
    ...
    if ($_FILES['pImg']['error'] === UPLOAD_ERR_OK) {
        ...
        if (is_uploaded_file($_FILES['pImg']['tmp_name']) && move_uploaded_file($_FILES['pImg']['tmp_name'], $targetFile)) {
            ...
            $query = "INSERT INTO product(prod_name,prod_currprice,prod_kg,prod_ml,prod_g,prod_category_id,prod_critical,prod_description,prod_voucher_id,prod_image,prod_added,prod_sell_onlline,prod_expirationStatus) 
            VALUES('$pname', '$pCprice', '$kg','$ml','$g', '$pcat', '$pcritical', '$pDescript', '$pVouch', '$pImg', '$currentDateTime','$SellOnlineTogler','$expirationStatus')";
